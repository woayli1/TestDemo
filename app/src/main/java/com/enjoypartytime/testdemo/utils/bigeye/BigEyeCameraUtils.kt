...
fun centerCropTextureRect(
    ...
    return when {
        oldRectRatio - targetRatio > 0.00001 -> {
            ...
            Point(x = newTopLeftX, y = topLeftPoint.y) to Point(
                x = newBottomRightX,
        ...
        targetRatio - oldRectRatio > 0.00001 -> {
            ...
            Point(x = topLeftPoint.x, y = newTopLeftY) to Point(
...
fun centerPoint(topLeftPoint: Point, bottomRightPoint: Point): Point {
    return Point(
        x = (topLeftPoint.x + bottomRightPoint.x) / 2.0f,
        y = (topLeftPoint.y + bottomRightPoint.y) / 2.0f
    )
}
...
fun Array<Point>.computeFaceTextureOval(): Oval {
    ...
    val topPoint = get(12)
    val centerPoint =
        Point((leftPoint.x + rightPoint.x) / 2.0f, (leftPoint.y + rightPoint.y) / 2.0f)
